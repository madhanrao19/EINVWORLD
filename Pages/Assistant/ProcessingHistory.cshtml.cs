...
namespace eInvWorld.Pages.Assistant
{
    ...
    /// Module="AI Assistant") and document captures (AuditLog, Action="DocumentCaptured"). Admins see
    /// every user's activity; everyone else sees only their own. No AI provider secrets are ever stored
    /// in either log, so nothing here needs masking.
    /// </summary>
    ...
    public class ProcessingHistoryModel : PageModel
    {
        ...
        public async Task OnGetAsync()
        {
            ...
            var captureQuery = _context.Set<AuditLog>().Where(a => a.Action == "DocumentCaptured");
