...
{
    public class CreateSBCNModel : PageModel
    {
        ...
        public int? PrimarySupplierId { get; set; } // Store Primary Supplier ID
        ...
        public void OnGet(string? invoiceNo = null, string? uuid = null)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Invoice not found for OnGet.");
                ...
                _logger.LogDebug("Populating Credit Note with InvoiceNo: {InvoiceNo}, UUID: {UUID}", existingInvoice.InvoiceNo, existingInvoice.UUID);
                ...
                Invoice = new InvoiceHeaderView
                {
                    ...
                    DocTypeCode = "12", // SelfBill Credit Note Doc Type
                ...
                _logger.LogDebug("Credit Note populated with {LineCount} lines. InvoiceNo: {InvoiceNo}, RefUUID: {RefUUID}", Invoice.InvoiceLines.Count, existingInvoice.InvoiceNo, Invoice.RefUUID);
            ...
            {
                _logger.LogError(ex, "OnGet exception loading Credit Note");
        ...
        public async Task<IActionResult> OnPostAsync(string action)
        {
            try
            {
                ...
                {
                    var errors = ModelState.Where(s => s.Value.Errors.Any())
                        .Select(s => $"{s.Key}: {string.Join(", ", s.Value.Errors.Select(e => e.ErrorMessage))}");
                ...
                // ✅ Calculate all invoice line amounts, including tax before saving
                foreach (var line in Invoice.InvoiceLines)
                {
                    line.CalculateAmounts();
                }

                // Validate supplier and customer
                ...
                {
                    case "generateJson":
                        _logger.LogDebug("Generating JSON for Invoice: {InvoiceNo}", Invoice.InvoiceNo);
                    ...
                    case "submitDocuments":
                        _logger.LogDebug("Submitting documents for Credit Note e-Invoice: {InvoiceNo}", Invoice.InvoiceNo);
            ...
            {
                _logger.LogError(ex, "Exception processing request");
        ...
        private bool SaveDraft(string invoiceJson, PartyInfo supplier, PartyInfo customer)
        {
            try
            {
                ...
                _logger.LogInformation("Draft saved at: {FilePath}", filePath);
        ...
        public async Task<IActionResult> OnPostSubmitDocumentsAsync(string invoiceNo)
        {
            ...
            try
            {
                _logger.LogInformation("Submitting Credit Note: {InvoiceNo}", invoiceNo);
                if (string.IsNullOrEmpty(invoiceNo))
                {
        ...
        private InputModels.InvoiceHeader CreateInvoiceHeader(PartyInfo supplier, PartyInfo customer)
        {
            ...
            _logger.LogDebug("Mapped UUID: {UUID}, RefUUID: {RefUUID} in CreateInvoiceHeader", invoiceHeader.UUID, invoiceHeader.RefUUID);
