...
| `SmartCapture` | Optional persisted/async Smart Capture, labelled "Create from Document" in nav (OFF; needs `DocumentCapture`+`AI:Enabled`). No application-level malware scanning — relies on file-type/signature validation, size/page/quota limits, and tenant-scoped storage instead; see deployment guide PART 17d. |
